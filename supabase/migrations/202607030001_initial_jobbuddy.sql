...
do $$ begin
  create type public.onboarding_state as enum ('resume_required', 'processing', 'review_required', 'preferences_required', 'ready');
exception when duplicate_object then null;
end $$;

do $$ begin
  create type public.resume_status as enum ('uploaded', 'processing', 'review_required', 'ready', 'failed', 'archived');
exception when duplicate_object then null;
end $$;
...
create index if not exists jobs_open_recent_idx on public.jobs(posted_at desc) where closed_at is null;
...
values
  ('free', 'Free', 0, '{"job_results_daily": 10, "assisted_applications_monthly": 3}'::jsonb),
  ('pro', 'Pro', 1900, '{"job_results_daily": null, "assisted_applications_monthly": 30}'::jsonb),
  ('power', 'Power', 4900, '{"job_results_daily": null, "assisted_applications_monthly": 100}'::jsonb)
...
create index if not exists usage_ledger_user_period_idx
  on public.usage_ledger(user_id, usage_type, occurred_at desc);
...
create policy "preferences_all_own" on public.job_preferences for all to authenticated
using ((select auth.uid()) = user_id) with check ((select auth.uid()) = user_id);
create policy "skills_all_own" on public.profile_skills for all to authenticated
using ((select auth.uid()) = user_id) with check ((select auth.uid()) = user_id);
create policy "experiences_all_own" on public.experiences for all to authenticated
using ((select auth.uid()) = user_id) with check ((select auth.uid()) = user_id);
create policy "educations_all_own" on public.educations for all to authenticated
using ((select auth.uid()) = user_id) with check ((select auth.uid()) = user_id);
...
using ((select auth.uid()) = user_id);
create policy "resume_extractions_select_own" on public.resume_extractions for select to authenticated
using ((select auth.uid()) = user_id);

create policy "companies_read_authenticated" on public.companies for select to authenticated using (true);
create policy "sources_read_authenticated" on public.job_sources for select to authenticated using (status = 'active');
...
create policy "matches_select_own" on public.job_matches for select to authenticated
using ((select auth.uid()) = user_id);
...
using ((select auth.uid()) = user_id);
create policy "application_answers_insert_own" on public.application_answers for insert to authenticated
with check ((select auth.uid()) = user_id);
create policy "application_answers_update_own" on public.application_answers for update to authenticated
using ((select auth.uid()) = user_id) with check ((select auth.uid()) = user_id);
create policy "application_events_select_own" on public.application_events for select to authenticated
using ((select auth.uid()) = user_id);
create policy "application_artifacts_select_own" on public.application_artifacts for select to authenticated
using ((select auth.uid()) = user_id);

create policy "subscriptions_select_own" on public.subscriptions for select to authenticated
using ((select auth.uid()) = user_id);
create policy "usage_select_own" on public.usage_ledger for select to authenticated
using ((select auth.uid()) = user_id);
